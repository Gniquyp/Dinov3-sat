...
class MSDeformAttnFunction(Function):
    ...
    def forward(ctx, value, value_spatial_shapes, value_level_start_index, sampling_locations, attention_weights, im2col_step):
        ...
        if _has_cuda_ext:
            ...
            ctx._use_pytorch = True
        return output

    @staticmethod
    @once_differentiable
    def backward(ctx, grad_output):
        if hasattr(ctx, '_use_pytorch') and ctx._use_pytorch:
            ...
            with torch.enable_grad():
                ...
                output = ms_deform_attn_core_pytorch(value, value_spatial_shapes, sampling_locations, attention_weights)
                grads = torch.autograd.grad(output, (value, sampling_locations, attention_weights), grad_output)
